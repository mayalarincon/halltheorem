...
definition finite_graph :: "'v digraph \<Rightarrow> bool"  where
  "finite_graph G \<equiv> finite (V[G]) \<and> finite (E[G])"
...
proof- 
  ...
  ultimately have  "finite_graph ?H" by(unfold finite_graph_def, auto)
  thus ?thesis  by(unfold subgraph_aux_def, auto)
qed
...
lemma diag_nat:
  shows "\<forall>yz.\<exists>x. (y,z) = diag x" 
  using enumeration_natxnat by(unfold enumeration_def,auto)

lemma Enum_Formulas_Vertices:
  assumes "\<exists>g. enumeration (g:: nat \<Rightarrow>'v)" 
  shows "\<exists>f. enumeration (f:: nat \<Rightarrow>('v \<times> nat)formula)"
proof-
  from assms obtain g where e: "enumeration (g:: nat \<Rightarrow>'v)" by auto  
  have  "enumeration ((\<lambda>m.(g(fst(diag m)),snd(diag m))):: nat \<Rightarrow>('v \<times> nat))"
  proof(unfold enumeration_def) 
    ...
    proof
      ...
      proof-
        ...
        from e have  "\<forall>w::'v. \<exists>n. w = (g n)" by(unfold enumeration_def, auto)
    ...
    qed
  qed
  thus "\<exists>f. enumeration (f:: nat \<Rightarrow>('v \<times> nat)formula)" 
    using EnumerationFormulasP1 by auto 
qed

(* Good replacing EnumeracionFormulasP1 above *)
...
proof(rule allI, rule impI)
  ...
  proof(rule ccontr)
    ...
    hence "t_v_evaluation I (atom (v, j)) = Ttrue" using CasosValor by blast
...
theorem satisfactible_coloring:
...
theorem coloring:
  assumes "is_graph (G::('vertices set) \<times> (('vertices \<times> 'vertices) set))"
  and "\<exists>g. enumeration (g:: nat \<Rightarrow>'vertices)"  
...
proof- 
  ...
  hence "satisfiable (\<T> G k)" using 
   Compacteness_Theorem[OF Enum_Formulas_Vertices[OF
  `\<exists>g. enumeration (g:: nat \<Rightarrow>'v)` ], of "(\<T> G k)"]
  by auto
  thus ?thesis using assms(1) satisfactible_coloring by blast
qed
end
